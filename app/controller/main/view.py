...
def user(username):
    ...
    page = request.args.get("page", 1, type=int)
    pagination = user.posts.filter_by(disable=False).order_by(
        Post.timestamp.desc()
    ).paginate(
        page, current_app.config.get("FLASK_USERPOST_PER_PAGE", 20), error_out=False
    )
    posts = pagination.items
    return render_template(
...
def admin_info_edit(username):
    ...
    form.permission.choices = [
        ...
        for permission in model_permission.query.order_by(
            model_permission.id.asc()
        ).all()
    ]
    if form.validate_on_submit():
        ...
        for new_permission in form.permission.data:
            new_permission = model_permission.query.filter_by(
                name=new_permission
            ).first()
    ...
    user_permissions = "".join(
        [permission.name for permission in user.permissions.all()]
    )
    return render_template(
        "main/adminedit.html", form=form, user_permission=user_permissions, user=user
    )
...
def post(postid):
    ...
    page = request.args.get("page", 1, type=int)
    pagination = post.comments.filter_by(disable=False).order_by(
...
def comment_submit(postid):
    ...
    if form.validate_on_submit():
        ...
        page = post.comments.count() // current_app.config.get(
            "FLASK_COMMET_PER_PAGE", 20
        ) + 1
...
def followers(username):
    ...
    pagination = user.followers.order_by(Follow.timestamp.desc()).paginate(
        page, current_app.config.get("FLASK_USER_PER_PAGE", 20), error_out=False
    )
    followers = [item.follower for item in pagination.items]

    return render_template(
        "/main/followers.html", users=followers, user=user, pagination=pagination
    )
...
def followings(username):
    ...
    return render_template(
        "/main/followings.html", users=followings, user=user, pagination=pagination
    )
