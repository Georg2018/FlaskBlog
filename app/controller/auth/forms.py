...
class RegisterForm(FlaskForm):
    ...
    password = PasswordField(
        ...
        validators=[
            DataRequired(),
            Length(8, 20),
            EqualTo("password2", message="The two password must matched."),
        ],
    )
...
class LoginForm(FlaskForm):
    identifier = StringField(
        "Email or username", validators=[DataRequired(), Length(1, 20)]
    )
...
class ChangePasswordForm(FlaskForm):
    ...
    new_password = PasswordField(
        ...
        validators=[
            DataRequired(),
            Length(8, 20),
            EqualTo("new_password2", message="Two new password must matched."),
        ],
    )
...
class ChangeMailForm(FlaskForm):
    ...
    new_email = StringField(
        "New email", validators=[DataRequired(), Length(1, 20), Email()]
    )
    submit = SubmitField("Submit")
