...
class UserAuthTestCase(unittest.TestCase):
	...
	def test_password_change(self):
		...
		response = self.client.post(url_for('auth.changepass'), data={
	...
	def test_change_email(self):
		...
		response = self.client.post(url_for('auth.changemail'), data={
			"password":"12345678",
	...
	def test_reset_password(self):
		...
		response = self.client.post(url_for('auth.authresetpass'), data={
			"username":"test"
			}, follow_redirects=True)
		self.assertTrue('We have seet a mail to you.' in response.get_data(as_text=True))

		token = user.generate_resetpass_token()
		response = self.client.post(url_for('auth.resetpass', token=token), data={
			"password":"87654321"
			}, follow_redirects=True)
